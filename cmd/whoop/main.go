...
import (
	...
	"github.com/stephensun/whoop-cli/whoop/auth"
	"github.com/stephensun/whoop-cli/whoop/output"
	"github.com/stephensun/whoop-cli/whoop/summary"
)
...
func main() {
	...
	opts := options{mode: mode, limit: *limit, start: *start, end: *end}
	var err error
	switch command {
	case "auth":
		err = runAuth(ctx)
...
Commands: auth, config diagnose, profile, body, cycles, recovery, sleep, workouts, brief, week, weekly
...
func storeAndCredentials(ctx context.Context) (auth.Store, string, string, error) {
	store := auth.DefaultStore()
	id := strings.TrimSpace(os.Getenv("WHOOP_CLIENT_ID"))
	secret := strings.TrimSpace(os.Getenv("WHOOP_CLIENT_SECRET"))
	var err error
	if id == "" {
		id, err = store.Get(ctx, "client_id")
		if err != nil && !errors.Is(err, auth.ErrNotFound) {
			return nil, "", "", err
		}
	}
	if secret == "" {
		secret, err = store.Get(ctx, "client_secret")
		if err != nil && !errors.Is(err, auth.ErrNotFound) {
			return nil, "", "", err
		}
	}
	if id == "" || secret == "" {
		return nil, "", "", errors.New("WHOOP client credentials are missing; configure WHOOP_CLIENT_ID/WHOOP_CLIENT_SECRET or the secure credential store")
	}
	return store, id, secret, nil
}
func newAPI(ctx context.Context) (*whoop.Client, error) {
	store, id, secret, err := storeAndCredentials(ctx)
	if err != nil {
		return nil, err
	}
	oauthClient, err := auth.New(auth.Config{ClientID: id, ClientSecret: secret, Store: store})
	if err != nil {
		return nil, err
	}
...
func runAuth(ctx context.Context) error {
	...
	client, err := auth.New(auth.Config{ClientID: id, ClientSecret: secret, Store: store})
	if err != nil {
		return err
	}
...
func runConfig(ctx context.Context, args []string, opts options) error {
	...
	d := diagnostics{Store: fmt.Sprintf("%T", store), ClientIDConfigured: os.Getenv("WHOOP_CLIENT_ID") != "" || idErr == nil, ClientSecretConfigured: os.Getenv("WHOOP_CLIENT_SECRET") != "" || secretErr == nil, RefreshTokenConfigured: refreshErr == nil}
